...
class BoundingBox: SCNNode {
    ...
    var extent = SIMD3<Float>(0.4, 0.2, 0.4)
    {
        didSet {
    ...
    func fitOverPointCloud(_ pointCloud: ARPointCloud, focusPoint: SIMD3<Float>?) {
        ...
        var filteredFilteredPoints: [SIMD3<Float>] = []
        ...
        for point in pointCloud.points {
            ...
            for otherPoint in pointCloud.points {
                if distance(point, otherPoint) < 0.01, abs(point.y - otherPoint.y) > 0.02 {
//                    nearbyPoints += 1
//                    if nearbyPoints >= 1 {
...
//                    }
        ...
        let minY = filteredPoints.min(by: { $0.y < $1.y })?.y ?? .zero
        let maxY = filteredPoints.max(by: { $0.y < $1.y })?.y ?? .zero
        let treshold = (maxY - minY) / 2
        filteredFilteredPoints = filteredPoints.filter { point in
            point.y < treshold
        }
        
        guard !filteredFilteredPoints.isEmpty else { return }
        
        var localMin = -extent / 2
        var localMax = extent / 2
        
        for point in filteredFilteredPoints {
...
//        extent = .init(sceneView.pointOfView!.worldUp.x, sceneView.pointOfView!.worldUp.y, sceneView.pointOfView!.worldUp.z)
